  ...
  try {
    ...
    if (session?.user?.id) {
      try {
        ...
        await logUserActivity(session.user.id, 'SCAN_STOP', 'Tool: deep-crawler' + (details ? ' - ' + details : ''));
      } catch (e) {}
    }
    ...
    const flaskResponse = await fetch(`${kaliToolsUrl}/api/scan/stop`, {
      method: 'POST',
    ...
    console.error('Stop deep crawler scan error:', error);
    return NextResponse.json(
